...
function Container(id, parentContainer) {
    ...
    function registerLoop(modules, instances) {
        ...
        var unreadyModules = modules.filter(function (module) {
            // if there are deps and some have not been registered ...
            if (module.deps.length > 0 && module.deps.some(function (dep) {
    ...
    var getInstances = function getInstances(depKeys, container) {
        return depKeys.map(function (depKey) {
            return container.get(depKey.trim());
        });
    };

    var getDepKeys = function getDepKeys(module) {
    ...
    var container = {
         ...
         */
        register: function register(modules) {
            var depList = Object.keys(modules).map(function (key) {
...
var Injector = exports.Injector = {
     ...
     */
    register: function register(modules, container) {
     ...
     * dependencies cannot be met
     */
    invoke: function invoke(module, container) {
        if (container === undefined) {
            container = containerMap.root;
        }
     ...
     */
    getInstance: function getInstance(key, container) {
        if (container === undefined) {
            container = containerMap.root;
        }
     ...
     */
    createContainer: function createContainer(id, parent) {
        if (containerMap[id]) {
            // a container with the specified id already exists
            // throw?
            console.warn('a container with the specified id already exists. ' + 'returning the pre-existing container');
        ...
        if (parent !== undefined) {
            if (typeof parent === 'string') {
                ...
                if (!containerMap[parent]) {
                    // no parent exists
                    console.error('Specified parent container ID `' + parent + '`' + ', but no container has been registered with that ID');
                ...
                console.error('Invalid argument. Second parameter must be either a valid' + ' container ID, or a registered container instance. Value was');
